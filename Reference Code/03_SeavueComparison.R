# Overview:
# Training ada model and predicting ada attributable effect on UST TNF-naive placebo participants.

# Outputs:
# * remission rates table
# * ADA Trial Baseline Characteristics Table
# * UST active vs UST placebo (ADA simulated) Baseline Characteristics Table
...
b_ada <- lmerTest::lmer(DRUG_REDUCTION ~ (1|TRIAL), data=ADA)
summary(b_ada)
ranef(b_ada)
...
covariate_list <- c('CDAI_BASELINE_CENT','AGE_CENT','BMI_CENT','CRP..mgL_CENT',
                    'SEX.MALE','HxOfTNFi','STEROID','IMMUNOMOD','LOC.ILEAL')    # remove YEAR_NORM
...
# subtract out ada attributable effect from CDAI_WEEK8 value
# ADA_WEEK8  = (CDAI_BASELINE - PLACEBO_ATTRIBUTABLE) - ADA_ATTRIBUTABLE
# (CDAI_BASELINE - PLACEBO_ATTRIBUTABLE) == CDAI_WEEK8
# PLACEBO_ATTRIBUTABLE == CDAI_REDUCTION
...
N_ada = 239 # N of ada data used to train ada model
